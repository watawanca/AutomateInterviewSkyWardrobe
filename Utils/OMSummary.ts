...
type HourlyData = NonNullable<ReturnType<WeatherResponse["hourly"]>>;
type DailyData = NonNullable<ReturnType<WeatherResponse["daily"]>>;
...
async function loadConfig(): Promise<WeatherConfig> {
  ...
  const configPath = getConfigPath("weather.config.json");
...
function getHourlyValues(hourlyData: HourlyData, index: number, variableName: string): number[] {
...
function getDailyInt64Value(dailyData: DailyData, index: number, dayIndex: number, variableName: string): bigint {
...
export async function runOMSummary(options: { write?: boolean } = {}): Promise<DaySummary> {
  ...
  if (write) {
    const outputPath = getDataPath("om_summary.json");
...
const isDirectRun = (() => {
  ...
  return base === "OMSummary.ts" || base === "OMSummary.js";
})();
if (isDirectRun) {
  runFromCli().catch((error) => {
    console.error("[OMSummary] Failed:", error instanceof Error ? error.message : error);
    process.exit(1);
  });
}
