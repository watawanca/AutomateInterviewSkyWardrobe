...
function getHourlyValues(hourlyData, index, variableName) {
...
function getDailyInt64Value(dailyData, index, dayIndex, variableName) {
...
const hourlyTimes = Array.from({ length: hourlyLength }, (_, i) => new Date((Number(hourly.time()) + i * hourly.interval() + utcOffsetSeconds) * 1000));
...
const dailyDates = Array.from({ length: dailyLength }, (_, i) => new Date((Number(daily.time()) + i * daily.interval() + utcOffsetSeconds) * 1000)
    .toISOString()
    .slice(0, 10));
